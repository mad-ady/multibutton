...
#we receive a path to a /dev/input device where we will get the events from
my %options;
my %keyMapping;
GetOptions(\%options, 'input|i=s', 'config|c=s');

usage() if(! -e $options{'input'} || ! -e $options{'config'});

my $config = Config::YAML->new(config => $options{'config'});

#configuration
my $updatePeriod = $config->{updatePeriod} || 200_000; #check keys every $updatePeriod microseconds
#convert updatePeriod in seconds
$updatePeriod = $updatePeriod/1_000_000;
...
#populate %keys with the name - code mapping from the linux kernel
parseKeyMapping();
#print Dumper(\%keyMapping);

my $input = Linux::Input->new($options{'input'});
my %lastPressedTS = (); #keep a record of keys being held down
my @buffer;

$logger->info("Starting multibutton instance");

while(1){
	my @events = $input->poll($updatePeriod);
        my %keyJustPressed = ();
        my $somethingWasPressed = 0;
        foreach my $event (@events){
                #we process only type=1 (EV_KEY)
                ...
                if($event->{value} == 1 || $event->{value} == 2){
                        ...
                        $somethingWasPressed = 1;
                        #we need to remember that this key was pressed just now and needs to be skipped in the next block
                        $keyJustPressed{$keyMapping{$event->{code}}}=1;
                }
                if($event->{value} == 0){
                        #key was released. Delete it from the lastPressedTS hash
                        delete $lastPressedTS{$keyMapping{$event->{code}}};
                }
        }
        
        if(scalar(keys %lastPressedTS)>0){
                #we have keys being pressed
                foreach my $key (keys %lastPressedTS){
                        #these are keys being held. We need to record the keypress in the buffer
                        next if (defined $keyJustPressed{$key}); #it was already added to the buffer
                        push @buffer, $key;
                        $somethingWasPressed = 1;
                }
        }
...
sub usage {
	print STDERR "Usage: $0 -i /dev/input/eventX -c /etc/multibutton.conf

-i|--input	Path to an input event file (/dev/input/event*, /dev/input/by-id/*, /dev/input/by-path/*)
...
sub parseKeyMapping {
	while(<main::DATA>){
		if(/\#define ([^\s]+)\s+([0-9x]+)/){
			...
			my $code = $2;
			if($code=~/x/){
				#code is in hex, convert it to decimal
				$code= sprintf("%d", hex($code));
			}
			$keyMapping{$code}=$label;
		}
	}
}
...
__END__
#The following list of keys is taken from the linux kernel at https://raw.githubusercontent.com/torvalds/linux/master/include/uapi/linux/input-event-codes.h
